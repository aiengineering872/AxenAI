...
const Navigation: React.FC<{
  ...
  onDashboard: () => void;
}> = ({ isScrolled, menuOpen, toggleMenu, onNavClick, onEnroll, isAuthenticated, onDashboard }) => (
  <header
      ...
      <div className="hidden items-center gap-10 lg:flex">
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated && (
            <button
              onClick={onDashboard}
              className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white/80 transition hover:text-white hover:bg-white/20 focus:outline-none"
            >
              My Dashboard
            </button>
          )}
      ...
      >
        <div className="flex flex-col gap-4 px-6 py-6 text-sm font-medium text-white/70">
          ...
          {isAuthenticated && (
            <button
              onClick={onDashboard}
              className="rounded-full border border-white/15 bg-white/5 px-6 py-2 text-sm font-semibold text-white/80 transition hover:text-white hover:bg-white/10 focus:outline-none"
            >
              My Dashboard
            </button>
          )}
      ...
      >
        {[
          ...
          },
          {
            ...
            highlights: ['Feature Engineering', 'CI/CD for ML', 'Observability'],
          },
        ].map((course, index) => (
          <motion.div
            key={course.title}
